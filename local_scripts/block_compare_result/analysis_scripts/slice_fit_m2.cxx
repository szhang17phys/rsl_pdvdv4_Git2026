...
void slice_fitTMP(const std::string& rsl){
    ...
    string output_path = "/Users/shuaixiangzhang/Work/current/FNAL_Work2024/rsl_analyses/new_20250127/block_compare_result/results/combined_3k/fitted_results/m2/";
...
//    TH2F* inputTH2F = (TH2F*)inputFile->Get("membrane2");//Only for RSL100---    
    ...
    for(int i=0; i<num; ++i){
        ...
        else if(rsl == "corsika_rsl700_abs15" && distances[i] == 510){
            FitVars varsTMP = vars;
            varsTMP.meanIni = 0.0;
            varsTMP.meanMin = 0.0;
            varsTMP.meanMax = 0.0;
            varsTMP.mpvIni = 130.0;
            tmpResults = CLG1(hists[i], outputFile, varsTMP, name); 
        }  
